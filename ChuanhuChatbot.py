...
my_api_key = "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"  # 在这里输入你的 API 密钥
    ...
    with gr.Row(scale=1).style(equal_height=True):
        with gr.Column(scale=5):
            ...
            with gr.Row(scale=1):
                ...
                delFirstBtn.visible = False
                delLastBtn = gr.Button("🗑️ 删除最新对话")
                delLastBtn.visible = False
                reduceTokenBtn = gr.Button("♻️ 总结对话")
                reduceTokenBtn.visible = False
            ...
            with gr.Column(min_width=50, scale=1):
                with gr.Tab(label="ChatGPT"):
                 ...
                 #   usageTxt = gr.Markdown(get_usage(my_api_key), elem_id="usage_display")
                    ...
                    use_streaming_checkbox.visible = False

                    use_websearch_checkbox = gr.Checkbox(label="使用在线搜索", value=False)
                    use_websearch_checkbox.visible = False
                    ...
                    language_select_dropdown.visible = False

                    index_files = gr.Files(label="上传索引文件", type="file", multiple=True)
                    index_files.visible = False

                    systemPromptTxt = gr.Textbox(
                        show_label=True,
                        placeholder=f"在这里输系统提示词...",
                        label="系统提示词",
                        value=initial_prompt,
                        lines=10,
                    ).style(container=False)
                    with gr.Accordion(label="加载提示词模板", open=True):
                        with gr.Column():
                            with gr.Row():
...
logging.info(
    colorama.Back.GREEN
    + "\n今日AI的温馨提示：访问 http://localhost:7860 查看界面"
...
if __name__ == "__main__":
    ...
    # if not running in Docker
        ...
        if authflag:
            ...
            demo.queue(concurrency_count=CONCURRENT_COUNT).launch(server_name="0.0.0.0", server_port=8866, share=False) # 可自定义端口
